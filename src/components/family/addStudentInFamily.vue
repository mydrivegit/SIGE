...
<div class="container col-10 pl-5">
    <div class="homecontainer w-100">
      <h4 class="text-nowrap mb-5 text-center w-100 form-control-sm headingWithSearchBar">
        <strong>Ajoutez élève dan le famile {{familyId.title}}</strong>
      </h4>
      <div class="row">
...
export default {
  ...
  computed: {
    ...
    ...mapGetters(['members', 'familyId']),
    filterbyrole () {
      if (this.members) {
        return this.members.filter(member => {
          return member.roleStudent && member.status && !member.memberToFamily
        })
      }
    },
    filterbyname () {
  ...
  methods: {
    selectedId (arg) {
      this.$store.dispatch('modifyMemberIdData', {id: arg, data: {familyId: this.$route.params.memberId, memberToFamily: true}})
        .then(res => {
          if (res.status === 201) {
